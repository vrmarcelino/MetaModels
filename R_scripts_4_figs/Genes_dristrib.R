...
# how abundance was calculated (mag abundance * gene counts in the MAG)
...
# species composition (to map MAGs to samples)
...
#write.csv(gene_by_sample, "3.0_gene_by_sample_complete.csv")

#sum(gene_by_sample["doxD", ] != 0)
...
# save it:
write.csv(gene_df, "3.0_gene_by_sample_filt.csv")
...
#options(scipen=10)
# think of a graphical representation (check if the results are congruent given the spp diversity effect?)


#loop through genes, get the p value of the correlation:
for (i in 1:length(genes)){
  target_gene <- genes[i]
  model1 <- lm(paste0(target_gene," ~ Diagnosis + spp_diversity"), data=gene_df)
  summary(model1)
...
## save table:
write.csv(store_Ps, "3.0_lm_genes.csv")
