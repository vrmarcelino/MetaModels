...
This script calculates the difference in global production and consumption of a given metabolite for each MAG,
so each spp appears only once in the network (as a producer or consumer, dependign on where they balance is), which
is different from MetModels_cc_create_network_from_list.py (which keeps flexible MAGs as both producer and consumer nodes).
...
### Calculate weighted flux (flux * rel_abundance)
exchanges_all['flux_weighted'] = exchanges_all['flux'] *  exchanges_all['abundance']
...
exchanges = exchanges_all.groupby(["taxon","metabolite"]).agg({'abundance':['mean'],'flux':['mean', 'count'], 'flux_weighted':['sum']})
exchanges.columns = ['_'.join(col) for col in exchanges.columns.values]
exchanges.reset_index(level=('taxon','metabolite'), inplace=True) # convert taxon and metabolite into columns
